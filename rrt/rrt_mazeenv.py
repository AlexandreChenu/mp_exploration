...
class RRT():
    def __init__(self, env, path, x_init, x_goal, budget, d_max):
        """
        ...
        self.Grid = Container([50,50], [40,40], [[-0.01,15.01],[-0.01,15.01]])
        self.Grid.init_grid()
    ...
    def expand(self):
        ...
        """
        x_rand = self.env.observation_space.sample() # TODO: adapt to gym environment
        x_rand = tuple(x_rand) # random selection of a state
        x_nearest = self.get_nearest(x_rand) # select nearest state in the explorationt tree
        ...
        x_new = self.steer(x_nearest, x_rand, d) # get by expansion
    ...
    def steer(self, start, goal, d):
        ...
        ## non-holonomic version
        v = np.random.uniform([-0.1,-0.1],[0.1,0.1])

        #### set state to selected node
        ...
        steered_point = self.env.state_act(v)
    ...
    def search(self, x_goal, iteration):
        ...
        f_cov_exp = open(self.path + "/coverage_expansion_rrt_" + str(iteration) +".txt", "w")
