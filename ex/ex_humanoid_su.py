...
class Ex:
    def __init__(self, env, path, feature = lambda x : x[:3], frameskip = 1, resolution = 0.1, stochastic=True, budget= 200):
        ...
        self.sim_memory = dict() # simulation states memory

        self.Grid = Coverage_grid([30,30,30], [10,10,10], [[-10.01,10.01],[-10.01,10.01],[-10.01,10.01]])
        self.Grid.init_grid()
    ...
    def save_sim(self, state, sim_state):
        state = tuple(state)
        # if not state in self.sim_memory:
        #     self.sim_memory[state] = []
        self.sim_memory[state] = sim_state
    ...
    def step(self, x_goal):
        ...
        # get the high dimensional state
        s = self.useLeastUsed().state

        # access the complexe simulation state given the high dimensional state
        sim_s = self.sim_memory[tuple(s)] # huge memory required if we use such sim_memory?

        # print("sim_s = ", sim_s)

        # set the environment to the complete simulation state
        self.env.set_state_sim(sim_s)

        # random sample of the action space
        a = self.env.action_space.sample()

        for _ in range(self.frame_skip):
            # make a step of the environment using the sampled actions
            sp, _, _, _ = self.env.step(a)
        ...
        self.save_sim(sp, self.env.get_state_sim())
        self.insert(sp)
        #self.Grid.add(self.feature(sp))

        print("self.feature(s) = ", self.feature(s))

        self.full_transitions.append([self.feature(s), self.feature(sp), a])
        self.transitions.append([self.feature(s),self.feature(sp)])
