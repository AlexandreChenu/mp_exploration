...
class Ex:
    def __init__(self, env, path, feature = lambda x : x, resolution = 0.1, stochastic=True, budget= 200):
        ...
        self.verbose = False
        self.transitions = []
        self.transitions_w_actions = {}
        self.budget = budget
        self.env = env
        self.path = path

        self.Grid = Coverage_grid([50,50], [40,40], [[-0.01,10.01],[-0.01,10.01]])
        self.Grid.init_grid()
    ...
    def step(self, x_goal):
        ...
        #v = np.random.uniform([-0.5,-0.5],[0.5,0.5])
        ...
        if b_valid:
            sp = self.env.state_act(v)
            #print("\n s = ", s)
            #print("v = ", v)
            #print("sp = ", sp)
            assert s[0] != sp[0] or s[1] != sp[1] or s[2] != sp[2] or s[3] != sp[3]
            self.Grid.add(sp)

            self.transitions.append([s,sp])
            # print("[s,sp] = ", [s,sp])
            self.transitions_w_actions[tuple([s[0], s[1], s[2], s[3], sp[0], sp[1], sp[2], sp[3]])] = v
    ...
    def can_connect_to_goal(self, goal_state):
        ...
        if np.linalg.norm(np.array(self.env.state[:2]) - np.array(goal_state)) <= 0.5:
            return True
        else:
            return False
